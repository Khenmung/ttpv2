          ...
          formattedData.forEach(item => {
            ...
            if (item.StudentClasses == undefined) {
              ...
              if (clsobj.length > 0) {
                ...
                var objsemester = this.Semesters.filter(s => s.MasterDataId == item.StudentClasses[0].SemesterId);
  ...
  AssignNameClassSection(pStudents) {
    ...
    pStudents.forEach(student => {
      ...
      if (student.StudentClasses && student.StudentClasses.length > 0) {
        ...
        var _SemesterObj = this.Semesters.filter(f => f.MasterDataId == student.StudentClasses[0].SemesterId)

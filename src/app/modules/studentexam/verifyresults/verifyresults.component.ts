...
export class VerifyResultsComponent implements OnInit {
          ...
          .subscribe((attendance: any) => {
            ...
            attendance.value.forEach(main => {
              ...
              this.StudentAttendanceList.push({
                ...
                //AttendanceStatus: this.AttendanceStatus.filter(f => f.AttendanceStatusId == main.AttendanceStatusId)[0].MasterDataName,
  ...
  ProcessVerify() {
    ...
    this.ExamStudentSubjectResult.forEach(d => {
      var _TotalPresent = 0;
      let _presentId = this.AttendanceStatus.filter(f => f.MasterDataName.toLowerCase() == 'present')[0].MasterDataId;
      var attendancelist = this.StudentAttendanceList.filter(f => f.StudentClassId == d["StudentClassId"])
        .filter(f => f.AttendanceStatusId == _presentId);
      
      _TotalPresent = attendancelist.filter(f => f.AttendanceStatusId == _presentId).length;
  ...
  GetExamStudentSubjectResults(pExamId, pClassId, pSectionId, pSemesterId) {
    ...
    this.dataservice.get(list)
      .subscribe((examComponentResult: any) => {
        ...
        var _examSubjectMarkComponentDefn = [];
        ...
        var _noOfSubjectForAStudent = clssubj.filter(allsubj => allsubj.SubjectCategoryId == _subjectCategoryMarkingId);
  ...
  GetSubjectComponents() {
    ...
    this.dataservice.get(list)
      .subscribe((data: any) => {
        ...
        data.value.forEach(e => {
          var obj = this.ClassSubjects.filter(s => s.ClassSubjectId == e.ClassSubjectId);
          if (obj.length > 0) {
            ...
            var selectHowManyObj = this.SubjectTypes.filter(f => f.SubjectTypeId == obj[0].SubjectTypeId)
            var selectHowMany = 0;
